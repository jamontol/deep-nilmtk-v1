...
def convert_hipe(hipe_path, output_filename, format="HDF", use_columns=["SensorDateTime", "P_kW"], metadata_path="metadata"):
    ...
    _convert(hipe_path, datastore,
             _hipe_measurement_mapping_func, "Europe/Berlin", use_columns=use_columns)
...
def _load_csv(filename, measurements, drop_duplicates=False, sort_index=False, use_columns=["SensorDateTime", "P_kW"]):
    print(f"Processing {filename}")
    df = pd.read_csv(filename, usecols=use_columns, index_col=0)

    df.index = pd.to_datetime(df.index, utc=True)
    df = df.tz_convert("Europe/Berlin")
    df[use_columns[1]] *= 1000  # convert kW to W
